...
let avg = sum / count;
console.log("AVG-total: ", avg);

// ข้อ 4 หาจำนวน member ว่ามีทั้งหมดกี่คน
let memberObj = new Set();
let count_member = 0;
for (let list of bills) {
  if (list.member === null) continue;
  if (!(list.member.name in memberObj)) {
    memberObj.add(list.member.name);
    // memberObj[list.member.name] = list.member.name;
    // count_member++;
...
// console.log(Object.keys(bills[0]).length);
// console.log(count_member);
console.log(memberObj.size);

// ข้อ 5 หาจำนวน total ในแต่ละ location
let total_location = {};
for (let list of bills) {
  if (list.location in total_location) {
    total_location[list.location] += list.total;
  } else {
    total_location[list.location] = 0;
    total_location[list.location] += list.total;
  }
}
...
let total_payment = {};
for (let list of bills) {
  if (!(list.paymentType in total_payment)) total_payment[list.paymentType] = 0;
  total_payment[list.paymentType] += list.total;
}
...
for (let list of bills) {
  if (list.member === null) continue;
  if (!(list.member.name in total_member)) total_member[list.member.name] = 0;
...
// ข้อ 9  ในแต่ละ location หา total sale, bill, average total
// จำแนกตาม transaction type
let locationInfo = {};
for (let list of bills) {
  if (!(list.location in locationInfo)) {
    locationInfo[list.location] = {};
    locationInfo[list.location].total_sale = 0;
    locationInfo[list.location].total_bill = 0;
  }

  locationInfo[list.location].total_sale += list.total;
  locationInfo[list.location].total_bill += 1;
  locationInfo[list.location].average_total =
    locationInfo[list.location].total_sale /
    locationInfo[list.location].total_bill;
}
